...
namespace NextGenSpice.Utils
{
    ...
    public class ParameterMapper<TParam>
    {
        ...
        {
            var prop = GetMappedProperty(mapping);
        ...
        {
            if (!settersByKey.TryGetValue(key, out var setter))
                throw new ArgumentException($"Parameter '{key}' is not mapped to any property");
            setter(Target, value);
        }
        ...
        {
            if (!settersByIndex.TryGetValue(index, out var setter))
